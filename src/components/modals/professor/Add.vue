    ...
    <template #body>
      <div class="grid-max-3 gap-4 mb-2">
        ...
        <div>
          ...
          <input v-model="form.password" type="password" class="base-input-gray" placeholder="RFC sin homoclave"/>
        </div>
      </div>
    </template>
    <template #options>
...
<script setup>
  ...
  form.value.password = computed(() => {
    return truncate(form.value.rfc, 10, false);
  });
  ...
  const addProfessor = async () => {
    ...
    success.value = null;
    //
    const url = import.meta.env.VITE_PROFESSORS;
    ...
    try {
      ...
      error.value = e.response.data.message || 'Error al guardar el profesor.';
    } 
    finally {
      loading.value = false;
    }
