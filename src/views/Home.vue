<template>
  <div v-if="certificates.length>0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    <div v-for="cert in certificates" :key="cert.id"
      ...
      <h3 class="text-xl font-semibold mb-2" :class="'text-'+color">{{ cert.name }}</h3>
...
<script setup>
  ...
  const certificatesEndpoint = import.meta.env.VITE_CERTIFICATES;
  const certificatesUrl = `${apiBaseUrl}${certificatesEndpoint}`;
  ...
  const getCertificates = async () => {
    ...
    error.value = null;
    try {
      const response = await axios.get(certificatesUrl, {
        headers: {
